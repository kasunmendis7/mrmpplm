...
/* Added by Samindu */
/* Migration HBFDP default hash type */
#define DEFAULT_MIGRATE_HBFDP_HASH_TYPE 0
/* 0: means sha1, 1: means md5, 3: murmur32 */
/* ---------------- */
...
/* Added by Samindu */
    DEFINE_PROP_SIZE("hbfdp-hash-type", MigrationState,
                      parameters.hbfdp_hash_type,
                      DEFAULT_MIGRATE_HBFDP_HASH_TYPE),
/* ---------------- */
...
/* Added by Samindu */
bool migrate_hbfdp(void)
{
	MigrationState *s = migrate_get_current();

	return s->capabilities[MIGRATION_CAPABILITY_HBFDP];
}

bool migrate_dtrack(void)
{
	MigrationState *s = migrate_get_current();

	return s->capabilities[MIGRATION_CAPABILITY_DTRACK];
}
/* ---------------- */
...
/* Added by Samindu */
uint64_t migrate_hbfdp_hash_type(void)
{
	MigrationState *s = migrate_get_current();
	
	return s->parameters.hbfdp_hash_type;
}
/* ---------------- */
...
MigrationParameters *qmp_query_migrate_parameters(Error **errp)
{
	...
	/* Added by Samindu */
	params->has_hbfdp_hash_type = true;
	params->hbfdp_hash_type = s->parameters.hbfdp_hash_type;
	/* ________________ */
...
bool migrate_params_check(MigrationParameters *params, Error **errp)
{
	...
	/* Added by Samindu */
	if (params->has_hbfdp_hash_type &&
			(params->hbfdp_hash_type < 0 || params->hbfdp_hash_type > 1 )) {
		error_setg(errp, QERR_INVALID_PARAMETER_VALUE,
				"hbfdp_hash_type",
				"a invalid hash type value");
		return false;
	}
	/* ________________ */
...
static void migrate_params_apply(MigrateSetParameters *params, Error **errp)
{
	...
	/* Added by Samindu */
	if (params->has_hbfdp_hash_type) {
		s->parameters.hbfdp_hash_type = params->hbfdp_hash_type;
	}
	/* ________________ */
	if (params->has_max_postcopy_bandwidth) {
