...
/* Added by Samindu */
static void kvm_slot_sync_dtrack_dirty_pages(KVMSlot *slot)
{
	ram_addr_t start = slot->ram_start_offset;
	ram_addr_t pages = slot->memory_size / qemu_real_host_page_size();

	cpu_physical_memory_set_dtrack_dirty_lebitmap(slot->dirty_bmap, start, pages);
}
/* ---------------- */
...
/* Added by Samindu */
static void kvm_physical_sync_dtrack_dirty_bitmap(KVMMemoryListener *kml, MemoryRegionSection *section)
{
	KVMState *s = kvm_state;
	KVMSlot *mem;
	hwaddr start_addr, size;
	...
	while (size) {
		...
		if (kvm_slot_get_dirty_log(s, mem)) {
			kvm_slot_sync_dtrack_dirty_pages(mem);
		}
		start_addr += slot_size;
		size -= slot_size;
	}
...
/* Added by Samindu */
static void kvm_log_dtrack_sync(MemoryListener *listener, MemoryRegionSection *section)
{
	KVMMemoryListener *kml = container_of(listener, KVMMemoryListener, listener);

	kvm_slots_lock();
	kvm_physical_sync_dtrack_dirty_bitmap(kml, section);
	kvm_slots_unlock();
}
/* ---------------- */
		...
		AddressSpace *as, int as_id, const char *name)
{
	...
	if (s->kvm_dirty_ring_size) {
		...
		/* Added by Samindu */
		kml->listener.log_dtrack_sync = kvm_log_dtrack_sync;
		/* ---------------- */
...
void kvm_init_cpu_signals(CPUState *cpu)
{
	...
	sigdelset(&set, SIGSEGV); //Segv Signal Test
	if (kvm_immediate_exit) {
