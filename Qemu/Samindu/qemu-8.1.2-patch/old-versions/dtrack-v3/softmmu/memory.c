...
/* Added by Samindu */
static void memory_region_sync_dtrack_dirty_bitmap(MemoryRegion *mr, bool last_stage)
{
	MemoryListener *listener;
	AddressSpace *as;
	FlatView *view;
	 ...
	 */
	QTAILQ_FOREACH(listener, &memory_listeners, link) {
		if (listener->log_dtrack_sync) {
			as = listener->address_space;
			view = address_space_get_flatview(as);
			FOR_EACH_FLAT_RANGE(fr, view) {
				if (fr->dirty_log_mask && (!mr || fr->mr == mr)) {
					MemoryRegionSection mrs = section_from_flat_range(fr, view);
					listener->log_dtrack_sync(listener, &mrs);
				}
			}
			flatview_unref(view);
			trace_memory_region_sync_dirty(mr ? mr->name : "(all)", listener->name, 0);
...
{
	memory_region_sync_dtrack_dirty_bitmap(NULL, last_stage);
}
/* ---------------- */
