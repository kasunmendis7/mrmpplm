...
static void intarray_to_intList(int64_t *array, intList **list) {
	for ( int i = 0; i <= 49; i++ ) {
...
static void populate_transfer_info(MigrationInfo *info, MigrationState *s)
{
	info->has_page_count = true;
	intarray_to_intList(s->page_count, &info->page_count);
	info->has_fake_dirty = true;
	intarray_to_intList(s->fake_dirty, &info->fake_dirty);
}
/* --------------- */
...
static MigIterateState migration_iteration_run(MigrationState *s)
{
	...
	/* Added by Samindu */
	printf("# Pages Selected to Transfer : %llu\n", pending_size/4096LLU);
...
#else
	if (((((uint64_t)(pending_size/4096LLU)) <= 5000LLU) || (iteration > 50))) {
		trace_migration_thread_low_pending(pending_size);
		migration_completion(s);
		return MIG_ITERATE_BREAK;
	}
...
static void *migration_thread(void *opaque)
{
	...
	/* Added by Samindu */
	page_count = g_try_malloc0(50 * sizeof(int64_t));
	fake_dirty = g_try_malloc0(50 * sizeof(int64_t));

	set_hash_digest_length(migrate_hbfdp_hash_type());
	/* ---------------- */
