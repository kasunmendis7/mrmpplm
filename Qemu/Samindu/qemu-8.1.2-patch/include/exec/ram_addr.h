...
static inline bool cpu_physical_memory_is_clean(ram_addr_t addr)
{
	...
	/* Added by Samindu */
    bool dtrack = cpu_physical_memory_get_dirty_flag(addr, DIRTY_MEMORY_DTRACK);
    return !(vga && code && migration && dtrack);
	/* ---------------- */
}
                                                               ...
                                                               uint8_t mask)
{
	...
	/* Added by Samindu */
	if (mask & (1 << DIRTY_MEMORY_DTRACK) &&
        !cpu_physical_memory_all_dirty(start, length, DIRTY_MEMORY_DTRACK)) {
        ret |= (1 << DIRTY_MEMORY_DTRACK);
    }
    /* ---------------- */
    return ret;
}
    ...
    WITH_RCU_READ_LOCK_GUARD() {
        ...
        while (page < end) {
			...
			if (likely(mask & (1 << DIRTY_MEMORY_DTRACK))) {
                bitmap_set_atomic(blocks[DIRTY_MEMORY_DTRACK]->blocks[idx],
                                  offset, next - page);
            }
			/* ---------------- */
			...
			for (k = 0; k < nr; k++) {
				if (bitmap[k]) {
					...
					if (global_dirty_tracking) {
						...
						/* Added by Samindu */
						qatomic_or(&blocks[DIRTY_MEMORY_DTRACK][idx][offset], temp);
						/* ---------------- */
...
bool cpu_physical_memory_test_dirty(ram_addr_t start, ram_addr_t length, unsigned client);

DirtyBitmapSnapshot *cpu_physical_memory_snapshot_and_clear_dirty (MemoryRegion *mr, hwaddr offset, hwaddr length, unsigned client);
...
static inline void cpu_physical_memory_clear_dirty_range(ram_addr_t start, ram_addr_t length)
{
	...
	/* Added by Samindu */
	cpu_physical_memory_test_and_clear_dirty(start, length, DIRTY_MEMORY_DTRACK);
	/* ---------------- */
}
...
/* Added by Samindu */
static inline bool cpu_physical_memory_sync_dtrack_dirty_page(RAMBlock *rb, ram_addr_t start, ram_addr_t length, unsigned long page)
{
	...
	unsigned long pageword = BIT_WORD((rb->offset + (page * TARGET_PAGE_SIZE)) >> TARGET_PAGE_BITS);
	unsigned long bits;
	...
	if (((word * BITS_PER_LONG) << TARGET_PAGE_BITS) == (start + rb->offset) && !(length & ((BITS_PER_LONG << TARGET_PAGE_BITS) - 1))) {
		...
		WITH_RCU_READ_LOCK_GUARD() {
			...
			*p = 0;
			if (src[idx][offset]) {
				bits = qatomic_rcu_read(&src[idx][offset]);
				*p |= bits;
			}
		}
	} else {
...
static inline void cpu_physical_memory_clear_dtrack_bitmap(RAMBlock *rb, ram_addr_t start, ram_addr_t length)
{
	ram_addr_t addr;
	unsigned long word = BIT_WORD((start + rb->offset) >> TARGET_PAGE_BITS);
	unsigned long *udtrack = rb->dtrack_bmap;

	/* start address and length is aligned at the start of a word? */
	if (((word * BITS_PER_LONG) << TARGET_PAGE_BITS) == (start + rb->offset) && !(length & ((BITS_PER_LONG << TARGET_PAGE_BITS) - 1))) {
		int k;
		int nr = BITS_TO_LONGS(length >> TARGET_PAGE_BITS);
		unsigned long * const *kdtrack;
		unsigned long idx = (word * BITS_PER_LONG) / DIRTY_MEMORY_BLOCK_SIZE;
		unsigned long offset = BIT_WORD((word * BITS_PER_LONG) % DIRTY_MEMORY_BLOCK_SIZE);
		unsigned long page = BIT_WORD(start >> TARGET_PAGE_BITS);

		kdtrack = qatomic_rcu_read(&ram_list.dirty_memory[DIRTY_MEMORY_DTRACK])->blocks;

		for (k = page; k < page + nr; k++) {
			if (kdtrack[idx][offset]) {
				qatomic_xchg(&kdtrack[idx][offset], 0);
			}
			udtrack[k] = 0;
