...
then
	...
	then 
		...
		sleep 10
		sshpass -p "primedirective" ssh -o StrictHostKeyChecking=no root@10.22.196.158 "/home/cloudnet/samindu/workloads/libmemcached-1.0.18/clients/memaslap -s 10.22.196.195:11211 -c 6 > $EXPERIMENT_DIR/memaslap_stat.log 2>&1 &"
		sleep 10
...
then
	 sshpass -p "primedirective" ssh -o StrictHostKeyChecking=no root@10.22.196.163 "bash /mnt/nfs/samindu/mrmpplm/Scripts/Migration/Triggers/Pre-Copy/precopy-compress-dst.sh"
fi

# sshpass -p "primedirective" ssh -o StrictHostKeyChecking=no root@10.22.196.162 "bash /mnt/nfs/samindu/mrmpplm/Scripts/Migration/Triggers/Pre-Copy/precopy-vm-migrate.sh $OPTIMIZATION $HASH_TYPE"
bash /mnt/nfs/samindu/mrmpplm/Scripts/Migration/Triggers/Pre-Copy/precopy-vm-migrate.sh $OPTIMIZATION $HASH_TYPE
