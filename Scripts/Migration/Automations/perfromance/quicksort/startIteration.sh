...
get-migration-details() {
	...
	if [[ $PING -eq 0 ]]; 
	then
		echo root@10.22.196.$IP:/home/${USER}/Desktop/quicksort_nop.log
		echo "$EXPERIMENT_DIR"
		sshpass -p "$PASSWORD" rsync -e "ssh -o StrictHostKeyChecking=no" -av --progress --no-o --no-g root@10.22.196.$IP:/home/${USER}/Desktop/quicksort_nop.log "$EXPERIMENT_DIR"
		check-error "rsync" $?
		sshpass -p "$PASSWORD" ssh -o StrictHostKeyChecking=no root@10.22.196.$IP "rm /home/$USER/Desktop/quicksort_nop.log"
		log "Migration:" 
		log "$MIGRATION"      
		log ""
	else
...
then
	sshpass -p "vmpassword" ssh -o StrictHostKeyChecking=no root@10.22.196.$IP "/home/vmuser/Desktop/quicksort > /home/vmuser/Desktop/quicksort_nop.log 2>&1 &" &
	sleep 10
